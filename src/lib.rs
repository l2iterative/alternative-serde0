...
pub use deserializer::{from_slice_compact, Deserializer};
pub use err::{Error, Result};
pub use serializer::{to_vec_compact, to_vec_compact_with_capacity, Serializer};
...
mod tests {
    ...
    use crate::{from_slice_compact, to_vec_compact};

    #[test]
    fn test_vec_round_trip() {
        let input: Vec<u64> = vec![1, 2, 3];
        let data = to_vec_compact(&input).unwrap();
        let output: Vec<u64> = from_slice_compact(data.as_slice()).unwrap();
        assert_eq!(input, output);
    }
    ...
    fn test_map_round_trip() {
        ...
        let data = to_vec_compact(&input).unwrap();
        let output: BTreeMap<String, u32> = from_slice_compact(data.as_slice()).unwrap();
        assert_eq!(input, output);
    }
    ...
    fn test_tuple_round_trip() {
        ...
        let data = to_vec_compact(&input).unwrap();
        let output: (u32, u64) = from_slice_compact(data.as_slice()).unwrap();
        assert_eq!(input, output);
    }
    ...
    fn test_mixed_tuple() {
        ...
        let output: TestType = from_slice_compact(&data).unwrap();
        assert_eq!(input, output);
    }
    ...
    fn test_edge_cases() {
        ...
        for i in 0..=5 {
            ...
            input.0 = vec![127u8; i];
            let data = to_vec_compact(&input).unwrap();
            let output: U8SeqThenU32 = from_slice_compact(&data).unwrap();
            assert_eq!(input, output);
        }
        ...
        for i in 0..=5 {
            ...
            input.1 = vec![127u8; i];
            let data = to_vec_compact(&input).unwrap();
            let output: U32ThenU8SeqT = from_slice_compact(&data).unwrap();
            assert_eq!(input, output);
        }
    }
}
